...
typedef struct
{
     ...
     * |        |          |1 = Capture Function source is from internal ACMP output signal, internal clock (LIRC, HIRC), or external clock (HXT, LXT).
     ...
     * |[1]     |TRGPWM    |Trigger EPWM/BPWM Enable Bit
     * |        |          |If this bit is set to 1, each timer time-out event or capture event can be as EPWM/BPWM counter clock source.
     * |        |          |0 = Timer interrupt trigger EPWM/BPWM Disabled.
     * |        |          |1 = Timer interrupt trigger EPWM/BPWM Enabled.
     * |        |          |Note: If TRGSSEL (TIMERx_TRGCTL[0]) = 0, time-out interrupt signal as EPWM/BPWM counter clock source.
     * |        |          |If TRGSSEL (TIMERx_TRGCTL[0]) = 1, capture interrupt signal as EPWM/BPWM counter clock source.
     ...
     * |        |          |0 = PWM counter event trigger DAC start conversion has not occurred.
